...
class Qzone:
#在类中定义函数，所以第一个参数永远是类的本身事例变量self
    #计算gtk
    def get_gtk(self):
    ...
    def get_friend(self):
        ...
        #得到好友list是[[name1,QQNum1],[name2,QQNum2]]格式
    ...
    def find_topic(self,qq,name):
        ...
        while True:
            ...
            pos += 20
            #访问说说页面的HTTP报文
            #urllib.parse.urlencode将所需要的信息转码成HTTP格式的内容
            url += urllib.parse.urlencode(data)
            res = requests.get(url, headers = header, cookies = cookie)
            print('读取' + name + '的第' + str(page) + '页说说完成')
            page += 1
            #匹配出_preloadCallback之后的内容
            #没有出现内容说明没有权限访问，file_denied在这个函数外面打开
            ...
            if msg['msglist'] == None:
                print('\n'+name+'的空间无更多说说'+'\n')
                return 0
            #爬取到的说说内容在msglist里面，是list类型；msglist[i]是字典类型
            #每一条说说是m
            for m in msg['msglist']:
                #记录共同好友的点赞记录
                self.write_like(m,qq,name)
                #如果评论数小于10，不需要点“查看全部评论”
                if m['cmtnum']<10:
                    #如果转发了说说并且没有配文字，而且原说说被删了，就会出现错误
                    if m['conlist'] is None:
                        continue
                    #写入评论信息
                    self.write_comment(m,qq,name)
                #如果评论数大于10，需要点开“查看全部评论”
                else:
                    data_more = {
                        'uin':qq,
                        'tid':m['tid'],
        ...
        if ('commentlist' in m.keys()):
            ...
            for comment in m['commentlist']:
                ...
                if comment['uin'] in qq_list:
                    file_comment.write(name+'|~|'+comment['name']+'|~|'+comment['content']+'\n')
            # 如果评论大于20条的话需要再进入新的界面
            # 一般说说评论不超过40条，这里使用40条作为最大数量
            if m['cmtnum'] > 20:
                data_more_20 = {
                    'uin': qq,
    ...
    def write_like(self,m, qq, name):
        ...
        for like in r['data']['like_uin_info']:
            if like['fuin'] in qq_list:
                file_like.write(name + '|~|' + like['nick'] + '|~|' + '喜欢' + '\n')
